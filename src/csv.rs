...
pub(crate) struct Row<'a>(&'a [Entry<'a>]);
...
impl<'a> Row<'a> {
    ...
    pub(crate) fn get(self, index: usize) -> Option<Entry<'a>> {
        self.0.get(index).copied()
    }

    pub(crate) fn entries(self) -> &'a [Entry<'a>] {
        self.0
    }
}
...
pub(crate) struct Table<'a>(&'a [Row<'a>]);

impl<'a> Table<'a> {
    pub(crate) fn parse(arena: &'a Arena, text: &'a str) -> Self {
        parse(arena, text)
    }
    ...
    pub(crate) fn get(self, index: usize) -> Option<Row<'a>> {
        self.0.get(index).copied()
    }

    pub(crate) fn rows(self) -> &'a [Row<'a>] {
        self.0
    }
}
...
pub(crate) fn parse_file<'a>(arena: &'a Arena, path: &str) -> Table<'a> {
    let src = arena.alloc_str(&std::fs::read_to_string(path).unwrap_or_default());
    parse(arena, &src)
}

pub(crate) fn parse<'a>(arena: &'a Arena, text: &'a str) -> Table<'a> {
