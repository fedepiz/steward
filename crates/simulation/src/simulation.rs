...
fn tick(sim: &mut Simulation, mut request: Request, arena: &Bump) -> Response {
    if let Some(req) = request.init.take() {
        *sim = Simulation::default();
        sim.dialog = Dialog::new();
        crate::init::init(sim, req);
    }

    if let Some(choice) = request.interaction_pick_option.take() {
        let mut interactions = std::mem::take(&mut sim.dialog);
        interactions.pick_choice(arena, sim, choice);
        sim.dialog = interactions;
    }

    let process_input = !sim.dialog.has_interaction();
...
fn resolve_intents(sim: &mut Simulation, arena: &Bump, intents: Intents) {
    ...
    if let Some((initiator, receiver)) = intents.begin_interaction {
        ...
        if sim.detections.collides(initiator, receiver) {
            let mut dialog = std::mem::take(&mut sim.dialog);
            // We flip the order, as the initiator is usually the player..
            dialog.begin(arena, sim, receiver, initiator);
            sim.dialog = dialog;
